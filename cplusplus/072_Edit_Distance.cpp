...
/*
For this solution, you need to tell why when word1[i-1] == word2[j-1], 
we can use min_distance[i-1][j-1] directly?
Is it possible that min_distance[i-1][j] is smaller than min_distance[i-1][j-1]?
The answers is NO. Since min_distance[i][j] records the minimal operations
to convert strA[0,i-1] to strB[0,j-1], and min_distance[i-1][j] has one more 
character than min_distance[i-1][j-1]. 
*/
class Solution {
public:
  int minDistance(string word1, string word2) {
