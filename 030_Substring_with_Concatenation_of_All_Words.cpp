/*
Solution 1:
The space complexity is easy to conclude: the major additional space usage is
counter and cur_counter,
both of which consumes O(words.size()) space.
For the time complexity, it depends on the two for loops. The outer loop will
loop for O(s_size) times
and the inner loop will loop for O(word_amount) times. So the time complexity is
at most O(s_size * word_amount).
*/
class Solution {
 public:
  vector<int> findSubstring(string s, vector<string>& words) {
    ...
    for (int i = 0; i + words_total_len <= s_size; ++i) {
      ...
      for (int j = i; remain_count > 0 && j + word_len <= s_size;
           j += word_len) {
        const std::string& cur_word = s.substr(j, word_len);
        if (--cur_counter[cur_word] == -1) break;
        --remain_count;
      }
